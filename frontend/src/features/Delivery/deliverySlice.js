...
export const createDelivery = createAsyncThunk('deliveries/create', async (deliveryData, thunkAPI) => {
    try {
...
//get deliveries
export const getDeliveries = createAsyncThunk('deliveries/getAll', async(_, thunkAPI) => {
    try {
        const token = thunkAPI.getState().auth.user.token
...
export const getOneDelivery = createAsyncThunk('deliveries/getOne', async(id, thunkAPI) => {
    try {
        const token = thunkAPI.getState().auth.user.token
...
//update delivery
export const updatedDelivery = createAsyncThunk('deliveries/updateOne', async(id, thunkAPI) => {
    try {
        const token = thunkAPI.getState().auth.user.token
...
//delete delivery
export const deleteDelivery = createAsyncThunk('deliveries/delete', async(id, thunkAPI) => {
    try {
        const token = thunkAPI.getState().auth.user.token
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(deleteDelivery.fulfilled, (state, action) => {
                state.isLoading = false
                state.isSuccess = true
                state.deliveries = state.deliveries.filter((pack) => 
